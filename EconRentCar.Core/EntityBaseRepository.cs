...
namespace EconRentCar.Core
{
      ...
      where T : class, IEntityBase, new()
    {
        ...
        {
            return GetAll(includeProperties: includedProps).AsEnumerable();
        }
        ...
        {
            IQueryable<T> query = _set.AsNoTracking();//.Where(x => x.Deleted == false);
        ...
        {
            IQueryable<T> query = (includeDeleted) ? _set.AsNoTracking().AsQueryable() : _set.AsNoTracking().AsQueryable();//.Where(x => x.Deleted == false).AsQueryable();
        ...
        public T Find(int id, string includeProperties = "")
        {
        ...
        {
            return _set.AsNoTracking().Where(predicate);//.Where(x => x.Deleted == false).Where(predicate);
        }
        ...
        {
            IEnumerable<T> entities = _set.AsNoTracking().Where(predicate);
